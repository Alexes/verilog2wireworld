''' 
	Command line tool to transform netlists in EDIF (LPM) format to WireWorld layout in MCell(?) format
'''
...
import sys	
edif_filename = sys.argv[1]
print 'Parsing', edif_filename

import shlex
s = shlex.shlex( file(edif_filename) )
...
# TODO Не будет ли проблем с тремя соединенными портами в WireWorldе? Можно поставить вентиль OR
print 'INSTANCES:'
for key in design_view['contents']['instances']:
	instance = design_view['contents']['instances'][key]
	print instance['name'] + ':', instance['library'] + '.' + instance['cell'] + '.' + instance['view']
print 'NETS:'
